    ...
    <div class="w-full h-[88%] bg-gray-200 flex items-center justify-center">
        <div class="w-[90%] h-5/6 bg-white rounded-xl pt-3 flex flex-col items-center">
            ...
            <div class="flex w-full h-4/5">
                <form action="{{route('product.edit',['product'=>$product->id])}}" method="post" class="w-full h-full flex flex-row-reverse">
                    @csrf
                    @method('put')
                    <div class="w-1/2 h-full flex flex-col items-end pr-20 relative">
                        <div class="w-full h-auto flex flex-row-reverse justify-start pt-4 mb-10">
                            ...
                            <input type="text" name="title" id="title" value="{{$product->title}}" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                            @if($errors->has('title'))
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20">
                        ...
                        <div class="w-full h-auto flex flex-row-reverse justify-start pt-4 mb-10">
                            ...
                            <select name="entity" id="entity" class="w-2/6 cursor-pointer outline-0 rounded border border-gray-400">
                                <option value=1 {{$product->entity==1?"selected":""}}>موجود</option>
                                <option value=0 {{$product->entity==0?"selected":""}}>ناموجود</option>
                            </select>
